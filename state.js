...
function loadConfig() {
    ...
    if(saved) {
        // Carga valores guardados, asegurando que el tiempo no sea nulo
        selectedTime = saved.time || 30;
        riskLevel = saved.risk || 3;
        flexMode = saved.flex || false;
        ...
        // Configuración inicial por defecto (Primera vez de uso)
        selectedTime = 30; 
        riskLevel = 3;    
        flexMode = false;
        neuralMode = true;
...
// Añadir en state.js (dentro del objeto config o debajo)
const volatilityShield = {
    criticalNoise: 45,      // Si el ruido supera esto, el bot se pone en alerta
    autoSniperMode: true,   // Forzar Nivel 3 (Sniper) en alta volatilidad
    panicStop: 60           // Ruido extremo: Apagado total preventivo
};